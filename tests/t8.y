...
%error-verbose
%lex-param {t8::ParseDriver* parser}
%locations
%define api.namespace {t8}
%parse-param {t8::ParseDriver* parser}
%parse-param {t8::AstNode** result}
...
A* A::parse(istream& input, ostream& errors) {
  return dynamic_cast<A*>(t8::parse(input, errors));
}
...
const char usage[] = "usage: t8 [input-file | --samples]\n";
...
int main(int argc, char** argv) {
  ...
  if (argc == 2 && std::string(argv[1]) == "--samples") {
    ...
    t8::ParseAndPrint(input, std::cout);
  }
  return 0;
}
